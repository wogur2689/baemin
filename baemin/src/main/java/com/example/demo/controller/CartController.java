...
import com.example.demo.util.FoodPriceCalc;
...
public class CartController {
	...
	@ResponseBody
	@PostMapping("/addCart")
	public CartList addCart(Cart cart, long storeId, String storeName, int delevveryTip, HttpSession session) {
		CartList cartList = (CartList) session.getAttribute("cartList");
		
		int totalPrice = FoodPriceCalc.foodPriceCalc(cart);
		
		System.out.print("카트 가격 계산 = " + totalPrice);
		
		if(cartList == null) {
			List<Cart> newCart = new ArrayList<>();
			cart.setTotalPrice(totalPrice);
			newCart.add(cart);
			cartList = new CartList(storeId, storeName, totalPrice, delevveryTip, newCart);
		} else {
		...
		System.out.print("cartList = " + cartList);
		return cartList;
	}
	/**
	 * 가게 방문시 장바구니 조회
